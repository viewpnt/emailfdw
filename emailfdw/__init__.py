"""
This foreign data wrapper (FDW) can be used to:

1. access mails from an IMAP mailbox (SELECT),
2. update which IMAP folder their associated with (labels -- UPDATE),
3. delete emails by purging them via IMAP (DELETE)
4. send emails over SMTP (INSERT)
...
The emailfdw tries its best to convert postgresql quals into imap filters.
...
The email FDW will fetch only what is needed by the query: you should thus avoid
...
class EmailFdw(ForeignDataWrapper):
    """A foreign data wrapper for IMAP, SMTP
    """

    def __init__(self, options, columns):
        super(EmailFdw, self).__init__(options, columns)

        self._imap_agent = None

        self.host = options.get('host', None)
        if self.host is None:
            log_to_postgres('You MUST set the imap host', ERROR)
    ...
    def insert(self, new_values):
        """Send an email
        """
        log_to_postgres(unicode('INSERTING'), WARNING)
        log_to_postgres(unicode(new_values), WARNING)

    def update(self, old_values, new_values):
        """Change the tag on an email
        """
        log_to_postgres(unicode('UPDATING'), WARNING)
        log_to_postgres(unicode(old_values), WARNING)
        log_to_postgres(unicode(new_values), WARNING)

    def delete(self, old_values):
        """Delete an email
        """
        log_to_postgres(unicode('DELETING'), WARNING)
        log_to_postgres(unicode(old_values), WARNING)
    ...
    def execute(self, quals, columns):
        ...
        try:
            ...
            log_to_postgres(unicode(self.imap_server_charset), WARNING)
            log_to_postgres(unicode(conditions), WARNING)
        ...
        if matching_mails:
            uids = list(compact_fetch(matching_mails))
            columns = list(col_to_imap.values())

            log_to_postgres(unicode(uids), WARNING)
            log_to_postgres(unicode(columns), WARNING)

            data = self.imap_agent.fetch(uids, columns)
